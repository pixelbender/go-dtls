...
func hashForServerKeyExchange(sigAndHash signatureAlgorithm, version uint16, slices ...[]byte) ([]byte, crypto.Hash, error) {
	...
	if sigAndHash.sign == signatureECDSA {
		return sha1Hash(slices), crypto.SHA1, nil
	}
	return md5SHA1Hash(slices), crypto.MD5SHA1, nil
}

func pickTLS12HashForSignature(sigType uint8, clientList []signatureAlgorithm) (uint8, error) {
	if len(clientList) == 0 {
		return hashSHA1, nil
	}

	for _, sigAndHash := range clientList {
		if sigAndHash.sign != sigType {
			continue
		}
		if isSupportedSignatureAndHash(sigAndHash, supportedSignatureAlgorithms) {
...
NextCandidate:
	...
	sigAndHash := signatureAlgorithm{sign: ka.sigType}

	if ka.version >= VersionDTLS12 {
		if sigAndHash.hash, err = pickTLS12HashForSignature(ka.sigType, clientHello.signatureAlgorithmes); err != nil {
			return nil, err
		}
	}
...
func (ka *ecdheKeyAgreement) processServerKeyExchange(config *Config, clientHello *clientHelloMsg, serverHello *serverHelloMsg, cert *x509.Certificate, skx *serverKeyExchangeMsg) error {
	...
	sigAndHash := signatureAlgorithm{sign: ka.sigType}
	if ka.version >= VersionDTLS12 {
		sigAndHash = signatureAlgorithm{hash: sig[0], sign: sig[1]}
		if sigAndHash.sign != ka.sigType {
			return errServerKeyExchange
		}
		sig = sig[2:]
		if len(sig) < 2 {
